...
запустить скриптовой файл проекта 'launch.sh' в директории проекта с помощью команды на разных платформах:
```
Linux: sudo bash launch.sh
Windows (с WSL): sudo bash launch.sh
```
3) Если у пользователя на локальной машине отсутствует Boost, то есть вариант загрузки и запуска библиотеки через платформу Docker, но для этого нужне будет иметь сам Docker на своём персональном компьютере или ноутбуке. Ниже представлен способ загрузки библиотеки через Docker после клонирования репозитория на двух ОС:
```
...
```
4) После успешного запуска скриптового файла 
